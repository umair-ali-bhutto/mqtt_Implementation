...
@Component("com.fuel.ws.classes.WsAccountCreation")
public class WsAccountCreation implements Wisher {
	...
	ResponseModel initRequest(RequestModel rm) {
		...
		try {
			...
			String[] responseProc = AGFuelUtil.fuelAccountProcess(rm.getCorpId(), "ADD_ACCOUNT", "LOV",
	...
	ResponseModel fetchRequest(RequestModel rm) {
		...
		try {
			...
			String selectedCompany = rm.getAdditionalData().get("selectedCompany").toString();

			String[] responseProc = AGFuelUtil.fuelProcAutoFetch(rm.getCorpId(), "ADD_ACCOUNT", null,
					userModel.getUserId(), userModel.getUserCode(), userModel.getGroupCode(), selectedCompany);
			...
			if (responseProc[0].equals("00")) {
				...
				JsonObject jsonObject = new Gson().fromJson(responseProc[2], JsonObject.class);

				company = new Gson().fromJson(jsonObject.getAsJsonArray("company"), companyList);

				HashMap<Object, Object> o = new HashMap<Object, Object>();
				o.put("companyData", company);
				response.setData(o);
				response.setCode("0000");
				response.setMessage("Success.");
			} else {
