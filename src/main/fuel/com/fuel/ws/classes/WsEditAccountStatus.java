...
@Component("com.fuel.ws.classes.WsEditAccountStatus")
public class WsEditAccountStatus implements Wisher {
	...
	ResponseModel initRequest(RequestModel rm) {
		...
		try {
			...
			String[] responseProc = AGFuelUtil.fuelAccountProcess(rm.getCorpId(), "ACCOUNT_STATUS_EDIT", "LOV",
			...
			account = new Gson().fromJson(new Gson().toJson(mdl.getData().get("accountstatus")), accountList);
			
			
			ArrayList<KeyValueModel> lst = new ArrayList<KeyValueModel>();			
			lst = new Gson().fromJson(new Gson().toJson(mdl.getData().get("accountnewstatus")), accountList);
...
//			lst.add(keyValue);
	...
	ResponseModel fetchRequest(RequestModel rm) {
		...
		try {
			...
			String[] responseProc = AGFuelUtil.fuelGetData(rm.getCorpId(), "ACCOUNT_STATUS_EDIT", "GET_DATA",
