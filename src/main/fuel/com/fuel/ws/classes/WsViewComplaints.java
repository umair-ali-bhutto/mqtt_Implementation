...
import com.ag.fuel.util.UtilReport;
...
@Component("com.fuel.ws.classes.WsViewComplaints")
public class WsViewComplaints implements Wisher {
	...
	public ResponseModel doProcess(RequestModel rm) {
		...
		try {
			...
			String complaintType = rm.getAdditionalData().get("complaintType") == null ? null
					: rm.getAdditionalData().get("complaintType").toString();

			String status = rm.getAdditionalData().get("status") == null ? null
					: rm.getAdditionalData().get("status").toString();

			int complId = rm.getAdditionalData().get("complId") == null ? 0
					: Integer.parseInt(rm.getAdditionalData().get("complId").toString());
			...
			if (!Objects.isNull(complaintId)) {
				...
				if (user.getGroupCode() != SuperUserGroupCode) {
					compLogs = complaintsService.fetchAllFuelComplaints(userCode, complaintType, status, complId,
							frDate, tDate, ctd.getCode(), rm.getUserid(), rm.getUserid());
				} else {
					compLogs = complaintsService.fetchAllFuelComplaints(userCode, complaintType, status, complId,
							frDate, tDate, ctd.getCode(), null, null);
				}
			...
			if (compLogs.size() != 0) {
				...
				for (Complaint com : compLogs) {
					...
					mdl.setAssignedTo(getAssignedTo(
							Integer.parseInt((com.getAssignedTo() != null) ? com.getAssignedTo() : "0"), userListAll));
