package com.generic.ws.classes;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import com.ag.generic.entity.UserLogin;
import com.ag.generic.model.BroadCastMessageModel;
import com.ag.generic.model.RequestModel;
import com.ag.generic.model.ResponseModel;
import com.ag.generic.service.BroadCastMessageDetailService;
import com.ag.generic.service.UserLoginService;
import com.ag.generic.service.Wisher;
import com.ag.generic.util.AgLogger;

@Component("com.generic.ws.classes.WsGetNotification")
public class WsGetNotification implements Wisher {
	@Autowired
	UserLoginService userLoginService;
	@Autowired
	BroadCastMessageDetailService broadCastMessageDetailService;

	public ResponseModel doProcess(RequestModel rm) {
		AgLogger.logInfo(rm.getUserid(), " CLASS CALLED " + rm.getMessage());
		ResponseModel response = new ResponseModel();

		try {
			UserLogin userLogin = userLoginService.validetUserid(Integer.parseInt(rm.getUserid()));
			if (userLogin != null) {
				List<BroadCastMessageModel> broadCastMessageModels = new ArrayList<BroadCastMessageModel>();

				response.setCode("0000");
				response.setMessage("SUCCESS");
				HashMap<Object, Object> obj = new HashMap<Object, Object>();
				broadCastMessageModels = broadCastMessageDetailService
						.searchBrdMsgByReadDateAndUserId(Integer.parseInt(rm.getUserid()));
				obj.put("NOTIFICATIONS", broadCastMessageModels);
				response.setData(obj);
			} else {

				response.setCode("0001");
				response.setMessage("USER NOT FOUND.");

				AgLogger.logInfo("USER NOT FOUND");
			}

		} catch (Exception ex) {

			response.setCode("9999");
			response.setMessage("Something Went Wrong, Please try again.");
			AgLogger.logInfo("EXCEPTION IN DoLogin :  " + ex);
			ex.printStackTrace();
		}

		return response;
	}

}
