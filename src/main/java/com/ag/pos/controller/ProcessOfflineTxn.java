...
@RestController
public class ProcessOfflineTxn {
	...
	@PostMapping({ "/serviceOfflinetxn", "/baflserviceOfflinetxn" })
	public JSONObject doProcessServiceTxn(@RequestBody String requestService, HttpServletRequest request)
		...
		try {
			...
			if (requestService.length() != 0) {
				...
				AgLogger.logInfo("OFFLINE TXN REQUEST RECIEVED .." + requestService);
				jmsSenderServiceImpl.sendOffline(requestService.toString());
				resp = "Acknowledged";
			...
			adt.setTxnType("serviceOfflinetxn");
			adt.setMid(mid);
			adt.setTid(tid);
