...
public class FuelQrPaymentAck {
	...
	public ResponseModel ackQrPayment(@RequestBody String requestService, HttpServletRequest request) throws Exception {
		...
		try {
			...
			e.printStackTrace();
			AgLogger.logerror(getClass(), "Exception In fuelQrPaymentAck: ", e);
			...
			adt.setTxnType("fuelQrPaymentAck");
		...
		AgLogger.logInfo("FuelQrPaymentAck RESPONSE: " + new Gson().toJson(response));
		return response;
	}

	@Transactional
	public ResponseModel responseServiceQrPayment(String xmls) {
		AgLogger.logInfo("FuelQrPaymentAck REQUEST: " + xmls);
		...
		try {
			...
			if (!Objects.isNull(userMdl)) {
				...
				AgLogger.logDebug("mid:", mid + "|tid:" + tid + "|serialNumber:" + serialNumber);
				...
				if (qp != null) {
					...
					mk.put("productName", "Gasoline");
					mk.put("productPrice", qp.getTxnAmount());
					mk.put("productQuantity", "1.0");
					mk.put("cardCompanyName", "BYCO TEST");

					mk.put("balanceAmount", "10.0");
					mk.put("balancePoints", "10.0");
