...
@Component("com.loy.adm.ws.classes.WsCorporateConfig")
public class WsCorporateConfig implements Wisher {
	...
	ResponseModel initRequest(RequestModel rm) {
		...
		try {
			...
			List<CorporateMaster> lst = corporateMasterService.fetchByCorpId(rm.getCorpId());

			if (lst.size() != 0) {
				response.setCode("0000");
				response.setMessage("SUCCESS.");
				HashMap<Object, Object> obj = new HashMap<Object, Object>();
				obj.put("canBeAdded", utilService.isEditRights(AppProp.getProperty("loy.corporate.screen.rights.add"),
						ukl.getGroupCode()));
				obj.put("canBeEdited", utilService.isEditRights(AppProp.getProperty("loy.corporate.screen.rights.edit"),
						ukl.getGroupCode()));
				obj.put("lstView", lst);
