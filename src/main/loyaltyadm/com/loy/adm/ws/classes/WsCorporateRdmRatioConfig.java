...
@Component("com.loy.adm.ws.classes.WsCorporateRdmRatioConfig")
public class WsCorporateRdmRatioConfig implements Wisher {


	@Autowired
	CorporateRdmRatioService corporateRdmRatioService;
	...
	ResponseModel initRequest(RequestModel rm) {
		...
		try {
			...
			List<CorporateRdmRatio> lst = corporateRdmRatioService.fetchAllByCorpId(rm.getCorpId());

			if (lst.size() != 0) {
				response.setCode("0000");
				response.setMessage("SUCCESS.");
				HashMap<Object, Object> obj = new HashMap<Object, Object>();
				obj.put("canBeAdded", utilService.isEditRights(AppProp.getProperty("loy.corportate.ratio.screen.rights.add"),
						ukl.getGroupCode()));
				obj.put("canBeEdited", utilService.isEditRights(AppProp.getProperty("loy.corportate.ratio.screen.rights.edit"),
						ukl.getGroupCode()));
				obj.put("lstView", lst);
