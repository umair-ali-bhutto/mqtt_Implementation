...
@Component("com.loy.adm.ws.classes.WsUserStatusStateConfig")
public class WsUserStatusStateConfig implements Wisher {

	@Autowired
	UserStatusStateService userStatusStateService;
	...
	ResponseModel initRequest(RequestModel rm) {
		...
		try {
			...
			List<UserStatusState> lst = userStatusStateService.fetchAllByCorpId(rm.getCorpId());

			if (lst.size() != 0) {
				response.setCode("0000");
				response.setMessage("SUCCESS.");
				HashMap<Object, Object> obj = new HashMap<Object, Object>();
				obj.put("canBeAdded", utilService.isEditRights(
						AppProp.getProperty("loy.user.status.state.screen.rights.add"), ukl.getGroupCode()));
				obj.put("canBeEdited", utilService.isEditRights(
						AppProp.getProperty("loy.user.status.state.screen.rights.edit"), ukl.getGroupCode()));
				obj.put("lstView", lst);
