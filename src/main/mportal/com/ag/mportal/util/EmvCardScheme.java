...
public enum EmvCardScheme {
	...
	//PAYPAK("PAYPAK", "^4[0-9]{6,}$", "A0 00 00 07 36"), //
	...
	BRADESCO("BRADESCO", null, "F0 00 00 00 03 00 01"), MIDLAND("Midland", null, "A0 00 00 00 24 01"), //
	 ...
	 *
	 * @param pAids   Card AID or RID
	 * @param pScheme scheme name
	 * @param pRegex  Card regex
	 */
	private EmvCardScheme(final String pScheme, final String pRegex, final String... pAids) {
		aids = pAids;
		aidsByte = new byte[pAids.length][];
		for (int i = 0; i < aids.length; i++) {
			aidsByte[i] = TapNPayUtil.fromString(pAids[i]);
		}
		name = pScheme;
