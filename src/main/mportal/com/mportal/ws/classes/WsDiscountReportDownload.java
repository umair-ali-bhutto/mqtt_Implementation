...
@Component("com.mportal.ws.classes.WsDiscountReportDownload")
public class WsDiscountReportDownload implements Wisher {
...
//				if (trxnModel != null) {
				...
				List<DMSReportReportModel> lstReportItems = discountMasterService.fetchDiscountTxnDetailsReport(midList,
						 frDate, tDate, discStatus, discNameList, 0, 0);
				...
				if (!Objects.isNull(lstReportItems) && !lstReportItems.isEmpty()) {
					...
					if (!Objects.isNull(blob)) {
						...
						return UtilAccess.generateResponse("0002", "Blob is Null.");
					}
				} else {
...
//				}
	...
	public String[] createReport(RequestModel rm, List<DMSReportReportModel> lstReportItems) {
...
//		File rootReport = new File("/home/umair/Desktop/" + rm.getUserid());
		String rootReportPath = AppProp.getProperty("discount.report.path") + rm.getUserid();
		File rootReport = new File(AppProp.getProperty("discount.report.path") + rm.getUserid());
		...
		String fileName = "DiscountDetails" + sd.format(d) + "." + reportFormat;
		File result = new File(rootReportPath + "/" + fileName);

		try {
			if (reportFormat.equals("csv")) {
				UtilReport.createDiscountDetailCsvFile(lstReportItems, result.getAbsolutePath());
			} else {
				UtilReport.createDiscountDetailCsvFile(lstReportItems, result.getAbsolutePath());
			}
