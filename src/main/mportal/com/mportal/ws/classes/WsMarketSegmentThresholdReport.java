...
@Component("com.mportal.ws.classes.WsMarketSegmentThresholdReport")
public class WsMarketSegmentThresholdReport implements Wisher {
	@Autowired
	...
	public ResponseModel doProcess(RequestModel rm) {
		...
		try {
			...
			String r1r2 = rm.getAdditionalData().get("r1r2") == null ? null
					: rm.getAdditionalData().get("r1r2").toString();
			...
			if (!Objects.isNull(merchantId) || (!Objects.isNull(fromdate) && !Objects.isNull(todate))
				...
				if (!Objects.isNull(r1r2) && r1r2.length() != 0) {
					mrktSeg = r1r2;
				}
				...
				int rows = Integer.parseInt(AppProp.getProperty("r1r2.report.rows").toString());

				int pageNumber = Integer.parseInt(currentPage);

				List<ReportModel> lstReportItems = txnDetailsService.fetchMarketSegmentReport(midList, tid, frDate, tDate,
						sett, btch, ath, mrktSeg, rows, pageNumber);
				if (!Objects.isNull(lstReportItems) && !lstReportItems.isEmpty()) {
					...
					if (flag.equals("Y")) {
						totalRecords = txnDetailsService.fetchMarketSegmentCount(midList, tid, frDate, tDate, sett, btch,
								ath, "DETAILS");
