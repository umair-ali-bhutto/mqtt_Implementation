...
@Component("com.mportal.ws.classes.WsMarketSegmentThresholdReportDownload")
public class WsMarketSegmentThresholdReportDownload implements Wisher {
		...
		try {
			...
			if (!Objects.isNull(merchantId) || (!Objects.isNull(fromdate) && !Objects.isNull(todate))
				...
				if (merchantName != null) {
					...
					List<ReportModel> lstReportItems = txnDetailsService.fetchMarketSegmentReport(midList, tid, frDate,
							tDate, sett, btch, ath, mrktSeg, 0, 0);
...
//		File rootReport = new File("/home/umair/Desktop/" + rm.getUserid());
		...
		String fileName = "R1R2MarketSegnebt" + sd.format(d) + "." + reportFormat;
		File result = new File(rootReportPath + "/" + fileName);

		try {
			if (reportFormat.equals("csv")) {
				UtilReport.createMarketSegmentCsvFile(lstReportItems, result.getAbsolutePath());
			} else {
				UtilReport.createMarketSegmentCsvFile(lstReportItems, result.getAbsolutePath());
			}
