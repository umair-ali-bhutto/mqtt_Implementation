...
@Component("com.mportal.ws.classes.WsOfflineTxnDetailsReportDownload")
public class WsOfflineTxnDetailsReportDownload implements Wisher {
	...
	public ResponseModel doProcess(RequestModel rm) {
		...
		try {
			...
			if (!Objects.isNull(merchantId) || (!Objects.isNull(fromdate) && !Objects.isNull(todate))
				...
				List<TxnSummary> trxnModel = offlineSaleService.fetchOfflineSaleTxnSummary(midList, flightList, tid, frDate,
						tDate, sett, btch, ath);

				if (trxnModel != null) {

					List<OfflineDetailsReportModel> lstReportItems = offlineSaleService.fetchOfflineSaleTxnDetailsReport(midList, flightList, tid,
							frDate, tDate, sett, btch, ath, 0, 0);

					if (!Objects.isNull(lstReportItems) && !lstReportItems.isEmpty()) {
						HashMap<Object, Object> obj = new HashMap<Object, Object>();
						String[] blob = createReport(rm, lstReportItems, trxnModel);
						...
						if (!Objects.isNull(flight) && flight.toLowerCase().equals("android")) {
	...
	public String[] createReport(RequestModel rm, List<OfflineDetailsReportModel> lstReportItems, List<TxnSummary> trxnModel) {
...
//		File rootReport = new File("/home/umair/Desktop/" + rm.getUserid());
		...
		try {
			if (reportFormat.equals("csv")) {
				offlineSaleService.createOfflineSummaryDetailCsvFile(trxnModel, lstReportItems, result.getAbsolutePath());
			} else {
				offlineSaleService.createOfflineSummaryDetailCsvFile(trxnModel, lstReportItems, result.getAbsolutePath());
			}
		} catch (Exception e) {
