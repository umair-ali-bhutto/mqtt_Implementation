...
@Component("com.mportal.ws.classes.WsTxnDetailsUpdDownload")
public class WsTxnDetailsUpdDownload implements Wisher {
		...
		try {
			...
			if (!Objects.isNull(merchantId) || (!Objects.isNull(fromdate) && !Objects.isNull(todate))
				...
				if (trxnModel != null) {
					...
					List<ReportModel> lstReportItems = txnDetailsService.fetchTxnDetailsReport(midList, tid, frDate,
							tDate, sett, btch, ath, 0, 0);
...
//		File rootReport = new File("/home/umair/Desktop/" + rm.getUserid());
		...
		try {
			if (reportFormat.equals("csv")) {
				UtilReport.createTxnSummaryDetailCsvFile(trxnModel, lstReportItems, result.getAbsolutePath());
			} else {
				UtilReport.createTxnSummaryDetailCsvFile(trxnModel, lstReportItems, result.getAbsolutePath());
			}
