...
@Component("com.mportal.ws.classes.WsTxnSummary")
public class WsTxnSummary implements Wisher {
				...
				if (trxnModel != null) {
					...
					if (!Objects.isNull(lstReportItems) && !lstReportItems.isEmpty()) {
						...
						for (ReportModel model : lstReportItems) {
							i++;
							try {
								PosEntryModeConfig posModeConfig = UtilAG.getPemConfigMap().get(model.getPem());
								if (!Objects.isNull(posModeConfig)) {
									model.setPemValue(posModeConfig.getPosEntryValue());
								} else {
									model.setPemValue("N/A");
								}
							} catch (Exception e) {
								model.setPemValue("N/A");
							}
