...
@Component("com.mportal.ws.classes.WsWebEcrBillPayment")
public class WsWebEcrBillPayment implements Wisher {
	...
	ResponseModel fetchRequest(RequestModel rm) {
		...
		try {
			...
			if (!rm.getChannel().equalsIgnoreCase("android") && !rm.getChannel().equalsIgnoreCase("ios")) {
				...
				acknmt = rm.getAdditionalData().get("ack") == null ? null
						: rm.getAdditionalData().get("ack").toString();
...
//			}
			...
			if (!Objects.isNull(acknmt) && !acknmt.equals("All")) {
				ack = acknmt;
			}
			...
			List<ReportModel> lstReportItems = ecrLogsService.fetchKEBillPaymentReport(midList, channelList, tid, frDate,
					tDate, sett, btch, ath, ack, rows, pageNumber);
			if (!Objects.isNull(lstReportItems) && !lstReportItems.isEmpty()) {
				...
				for (ReportModel model : lstReportItems) {
					String type = model.getType().equalsIgnoreCase("0000|Success") ? "Y"
						    : model.getType().equalsIgnoreCase("0006|Something Went Wrong.") ? "N"
						    	    : null;
					
					 LocalDate date = LocalDate.parse(model.getMerchantName().substring(0, 10));
				     String formattedDate = date.format(DateTimeFormatter.ofPattern("MMM-yyyy"));
					i++;
					model.setSerialNumber(i + "");
					model.setType(type + "");
					model.setMerchantName(formattedDate + "");
...
//				if (flag.equals("Y")) {
//					totalRecords = ecrLogsService.fetchBillPaymentDetailsCount(midList, channelList, tid, frDate, tDate,
//							sett, btch, ath,ack, "DETAILS");
