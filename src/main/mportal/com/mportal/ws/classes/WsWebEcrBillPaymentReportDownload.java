...
@Component("com.mportal.ws.classes.WsWebEcrBillPaymentReportDownload")
public class WsWebEcrBillPaymentReportDownload implements Wisher {
	...
	public ResponseModel doProcess(RequestModel rm) {
		...
		try {
			...
			String acknmt = rm.getAdditionalData().get("ack") == null ? null
...
//		File rootReport = new File("/home/umair/Desktop/" + rm.getUserid());
		...
		String fileName = "BillPaymentDetails" + sd.format(d) + "." + reportFormat;
		File result = new File(rootReportPath + "/" + fileName);

		try {
			if (reportFormat.equals("csv")) {
				ecrLogsService.createKEBillPaymentTxnDetailCsvFile(lstReportItems, result.getAbsolutePath());
			} else {
				ecrLogsService.createKEBillPaymentTxnDetailCsvFile(lstReportItems, result.getAbsolutePath());
			}
		} catch (Exception e) {
