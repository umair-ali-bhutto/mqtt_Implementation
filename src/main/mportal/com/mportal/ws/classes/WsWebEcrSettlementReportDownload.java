...
@Component("com.mportal.ws.classes.WsWebEcrSettlementReportDownload")
public class WsWebEcrSettlementReportDownload implements Wisher {
	...
	public String[] createReport(RequestModel rm, List<ReportModel> lstReportItems) {
		...
		try {
			if (reportFormat.equals("csv")) {
				ecrLogsService.createSettlementTxnDetailCsvFile(lstReportItems, result.getAbsolutePath());
			} else {
				ecrLogsService.createSettlementTxnDetailExcelFile(lstReportItems, result.getAbsolutePath());
			}
		} catch (Exception e) {
